 ...
 * main.cpp
 *
 * A (NOT) working example for Toolbox::NeuralNetwork::Recurrent
 */
...
//////////////////////////////////////////////////////////////////////////////
/*
using Toolbox::NeuralNetwork::RecurrentNucleus;	// From <Toolbox/NeuralNetwork/Recurrent.hpp>
using Toolbox::NeuralNetwork::RecurrentNeuron;

using Toolbox::NeuralNetwork::Neuron;			// From <Toolbox/NeuralNetwork/Neuron.hpp>
using Toolbox::NeuralNetwork::LabeledNeuron;

using Toolbox::NeuralNetwork::Ganglion;			// From <Toolbox/NeuralNetwork/Ganglion.hpp>

using Toolbox::NeuralNetwork::Recurrent;		// From <Toolbox/NeuralNetwork/Recurrent.hpp>
...
const size_t RecurrentMemorySize = 10;
typedef tRecurrentNeuron< tRecurrentNucleus<Default::tNeurotransmitter>, RecurrentMemorySize >	tRecurrent10Neuron;
...
void PrintNeuron( typename _tNeuron::Ptr neuron, unsigned int indent_spaces = 2 )
{
	typedef tLabeledNeuron< _tNeuron >		ttLabeledNeuron;
	...
	typedef tRecurrent10Neuron	tExpectedType;
	auto RecurrentMemory = std::dynamic_pointer_cast< tExpectedType >( neuron );
	bool First = true;

	if ( RecurrentMemory )
	{
		std::cout << Indent << "Memory (" << RecurrentMemory->Memory.size() << "): ";

		bool First = true;
		for ( auto m = RecurrentMemory->Memory.begin(), m_end = RecurrentMemory->Memory.end();
			  m != m_end;
			  ++m )
		{
			if ( !First )
		...
		std::cout << Indent << "Weight (" << RecurrentMemory->MemoryWeight.size() << "): ";

		First = true;
		for ( auto m = RecurrentMemory->MemoryWeight.begin(), m_end = RecurrentMemory->MemoryWeight.end();
			  m != m_end;
			  ++m )
		{
			if ( !First )
	...
	typename ttLabeledNeuron::Ptr ln;

	First = true;
	for ( auto d = neuron->Dendrites.begin(), d_end = neuron->Dendrites.end(); d != d_end; ++d )
	{
		ln = std::dynamic_pointer_cast< ttLabeledNeuron >( (d->first).lock() );
	...
	{
		ln = std::dynamic_pointer_cast< ttLabeledNeuron >( a->lock() );
...
void PrintGanglion( typename _tGanglion::Ptr network, unsigned int indent_spaces = 2 )
{
	if ( !network )
	...
	for ( auto i = network->Input.begin(), i_end = network->Input.end(); i != i_end; ++i )
		PrintNeuron< typename _tGanglion::ttNeuron >( i->second, indent_spaces + 2 );
	...
	for ( auto l = network->Hidden.begin(), l_end = network->Hidden.end(); l != l_end; ++l )
	{
		...
		for ( auto h = l->second.begin(), h_end = l->second.end(); h != h_end; ++h )
			PrintNeuron< typename _tGanglion::ttNeuron >( h->second, indent_spaces + 4 );
	}

	std::cout << Indent << "-- Outputs --" << std::endl;

	for ( auto o = network->Output.begin(), o_end = network->Output.end(); o != o_end; ++o )
		PrintNeuron< typename _tGanglion::ttNeuron >( o->second, indent_spaces + 2 );
...
int main( int argc, char *argv[] )
{
    try
    {
		...
		typedef tRecurrent< RecurrentMemorySize >	ttRecurrent;
		typename ttRecurrent::Ptr XORNet = std::make_shared< ttRecurrent >();
		...
		PrintGanglion< ttRecurrent >( XORNet );

		//
		// Prepare Training Data
		// Use our prepared XORData above to populate our XORSet
		//
		tTrainingSet< ttRecurrent > XORSet( *XORNet );			// Copy the inputs and outputs from our network
